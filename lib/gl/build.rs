extern crate gl_generator;

use gl_generator::{Registry, Fallbacks, StructGenerator, DebugStructGenerator, Api, Profile};
use std::env;
use std::fs::File;
use std::path::Path;

// Da build script
fn main() {
    // OUT_DIR env var comes from cargo build toolchain
    let out_dir = env::var("OUT_DIR").unwrap();

    // This script will create $OUT_DIR/bindings.rs
    let mut file_gl = File::create(&Path::new(&out_dir).join("bindings.rs")).unwrap();

    // Use gl_generator crate's code generation tool
    let registry = Registry::new(Api::Gl, (4, 5), Profile::Core, Fallbacks::All, [
        "GL_NV_command_list", // additional extension we want to use
    ]);

    // Use DebugStructGenerator if this is a debug build, otherwise StructGenerator
    if env::var("CARGO_FEATURE_DEBUG").is_ok() {
        registry.write_bindings(
            DebugStructGenerator,
            &mut file_gl
        ).unwrap();
    } else {
        registry.write_bindings(
        StructGenerator,
        &mut file_gl
    ).unwrap();
    }
}
